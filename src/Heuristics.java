...
public class Heuristics {
	 ...
	 * @return sum of distances for player and diamonds
	 */
	public double calculate(State s, String method) {
		HashSet<Coordinate> diamonds = s.diamonds;
		double sum = 0;
		
		//get distance from player to diamonds, and add to sum
		Coordinate player = s.player;
		double playerMin = getDist(player, diamonds, method);
		sum+= playerMin;
		
		//get distance from diamonds to goals, add each minimum distance to the sum
